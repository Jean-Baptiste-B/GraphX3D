    ...
    private TriangleMesh createFrustum(float majorRadius, float minorRadius, float height, int level) {
        ...
        if (level == 0) {
            ...
            if (getSectionType() != TriangleMeshHelper.SectionType.CIRCLE) {
                div = getSectionType().getSides() * (div / getSectionType().getSides() + 1);
            }
            ...
            // u right ,v up
            for (int i = 0; i <= div; i++) {
                baseTexCoords[2 * i] = (rBase + i * rectBase / div) / L;
                baseTexCoords[2 * i + 1] = (2f * rTop + h) / H;
            }
            for (int i = 0; i <= div; i++) {
                baseTexCoords[2 * div + 2 * i + 2] = (rBase + i * rectTop / div) / L;
                baseTexCoords[2 * div + 2 * i + 3] = 2f * rTop / H;
            }
            for (int i = 0; i <= div; i++) {
                double ang = i * 2d * Math.PI / div;
                double pol = polygonalSection(ang);
